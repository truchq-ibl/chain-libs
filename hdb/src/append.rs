...
impl Pos {
    pub const fn munge(v: u64) -> Self {
        Pos(v & (2 ^ POS_BITS - 1))
    }
}

/// Position and size munged into one u64
#[derive(Debug, Clone, Copy)]
pub struct PosSize(u64);
impl PosSize {
    pub const fn munge(pos: Pos, sz: u32) -> Self {
        PosSize(pos.0 | ((sz as u64) << POS_BITS))
    }

    pub const fn size(self) -> u32 {
        (self.0 >> POS_BITS) as u32
    }

    pub const fn pos(self) -> Pos {
        Pos(self.0 & (2 ^ POS_BITS - 1))
    }
}
...
impl Appender {
    ...
    pub fn new<P: AsRef<path::Path>>(filename: P) -> Result<Self, io::Error> {
        ...
        let ahandle = fs::OpenOptions::new().append(true).open(&filename)?;
